classdef WalkingCaneWanderer
    %WalkingCaneWanderer Robot driver using Vector Field Histogram
    %method.
    % w = WalkingCaneWanderer(laserSub, tf, velPub, velMsg) creates a
    ...
    %
    %   % Construct WalkingCaneWanderer class
    %   wanderHelper = WalkingCaneWanderer(laserSub, sensorTransform, velPub, velMsg);
    %
    ...
    methods
        function obj = WalkingCaneWanderer(laserSub, tf, velPub, velMsg)
        ...
        function wander(obj)
            ...
            if ~isnan(steerDir)
                desiredV = 0.1;
                w = WalkingCaneComputeAngularVelocity(steerDir, 0.3);
            else
                desiredV = 0.0;
                w = 0.3;
            end
